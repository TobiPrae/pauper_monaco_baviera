    ...
    with tab_record:
        def record_form(m, label):
            ...
            with st.expander(f"{'🔴' if not any(g.winner for g in m.games) else '🟢'} {label}: {p1} vs {p2}"):
                ...
                if can_edit:
                    match_link_val = st.text_input("Match Link", value=getattr(m, "match_link", "") or "", key=f"p_link_{m.id}")
                    if st.button("Save", key=f"p_save_{m.id}", use_container_width=True):
                        if start is None:
                            st.error("Please select a starting player.")
                        else:
                            client.update_match(m.id, games=[{'winner': 'A' if g==p1 else ('B' if g==p2 else None)} for g in (g1,g2,g3)], starting_player=start, match_type=m.match_type, match_link=match_link_val)
                            st.rerun()
