...
st.write("View all selected league data in one consolidated table.")
...
hall_rows = []
for league in sorted(selected_leagues, key=lambda l: _parse_date(getattr(l, 'end_date', None)) or pd.Timestamp.min, reverse=True):
...
if hall_df.empty:
    st.info("No Hall of Fame data found for the selected leagues.")
else:
